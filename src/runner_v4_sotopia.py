"""Runner v4 (Sotopia variant): within-decision-point per-message attribution
on the SOTOPIA-style social-negotiation env.

Mirrors runner_v4.py exactly except:
  * env is SotopiaEnv (3 characters ALEX/BLAKE/CASEY, rotating scenario)
  * target_phase is `PHASE_1` (the first negotiation round; PHASE_0 has no
    incoming press yet so attribution is undefined there)
  * phase-name is obtained via env._phase() (no diplomacy.Game)
  * orders are short string labels (one per character); the canonical fine
    representation is just the action string, coarse is the same (Sotopia
    has no fine/coarse split).
"""
...
from .sotopia_env import SotopiaEnv, SOTOPIA_POWERS
...
def _make_env(scenario="sotopia"):
    return SotopiaEnv()


def _orders_to_string(orders):
    """Canonical fine-grained representation for KL estimation (Sotopia)."""
    return ";".join(sorted(str(o) for o in orders))


def _orders_to_coarse(orders):
    """Coarse: same as fine on Sotopia (action vocab is already coarse)."""
    return _orders_to_string(orders)
...
def _sample_with_traces(arch, power, obs, n, seed_base, temperature, incentive="C"):
    ...
    def one(i):
        # Sotopia: GPT-4o is highly deterministic under (seed, T) so we
        # need real per-sample variation. We use a large-stride distinct
        # seed per sample AND disable the local cache (which keyed on the
        # exact seed and would otherwise replay a single deterministic
        # answer on every future run).
        eff_seed = (seed_base * 982451653 + i * 2147483647) & 0x7FFFFFFF
        act = agent.act(obs, seed=eff_seed, temperature=temperature,
                        use_cache=False)
        return (
            _orders_to_string(act["orders"]),
            _orders_to_coarse(act["orders"]),
